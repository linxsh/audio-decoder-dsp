		...
		while (ogg_sync_pageout(&opusDec->oy, &opusDec->og) == 1) {
			...
			while (ogg_stream_packetout(&opusDec->os, &opusDec->op) == 1) {
				...
				if (opusDec->packet_count == 0) {
					...
					if(err != OPUS_OK){
						log_printf(OPUS_DEC_MODULE, LEVEL_ERRO,
								"%s %d: create failed\n", __FUNCTION__, __LINE__);
						opusDec->state = OPUS_SYNC_PAGEOUT;
						return -1;
					}
					log_printf(OPUS_DEC_MODULE, LEVEL_INFO,
					...
					//trans out_save_buf to tmp32Buffer
						...
						ext_reg_set(&opusDec->w_ctrl->SAMPLE_RATE, OPUS_SAMPLE_RATE);
						ext_reg_set(&opusDec->w_ctrl->CHANNELS,    0x2);
						ext_reg_set(&opusDec->w_ctrl->ERROR_CODE,  0x0);
					} else {
						ext_reg_set(&opusDec->w_ctrl->ERROR_CODE,  (-88));
					}
